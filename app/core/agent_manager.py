...
from ..agent_hub.annotation_graph.annotated_graph import Graph
from ..agent_hub.annotation_graph.schema_handler import SchemaHandler
from ..agent_hub.rag.rag import RAG
from ..agent_hub.rag.utils.web_search import SimpleWebSearch
from ..prompts.conversation_handler import conversation_prompt
from ..summarizer import Graph_Summarizer
from ..agent_hub.hypothesis_generation.hypothesis import HypothesisGeneration
...
class AgentState(TypedDict):
    ...
    e2b_response: Optional[Dict[str, Any]]
    action_files: Optional[List[str]]
    e2b_retry_counts: Dict[str, int]
...
class AgentManager:
    ...
    def e2b_executor(self, state: AgentState) -> Dict[str, Any]:
        """
        Action-track agent: dynamic code execution via E2B.

        Flow (per the E2B architecture doc):
          1. Get/create a per-user sandbox via SessionManager (variables persist across turns)
          2. LLM generates Python code from the step input
          3. Execute code in the sandbox
          4. Critic validates output
          5. On failure: feed traceback back to LLM and retry (max 5x)
          6. On success: return stdout + plots + file list to the agent chain
        """
        task = state.get("step_input") or state["user_query"]
        user_id = state["user_id"]
        ...
        error_context = None
        max_attempts = 5
        if sync_errors and not available_files:
            error_context = "\n".join(sync_errors)

        for attempt in range(max_attempts):
            logger.info(f"E2B attempt {attempt + 1}/{max_attempts} for user {user_id}")

            code = self.e2b_code_generator.generate(
                task_for_generation,
            ...
            if is_valid:
                ...
                RichLogger.log_agent_complete("e2b_executor", summary[:200])
                return {
                    "e2b_response": {
                        "text": summary,
                        "stdout": stdout,
                    ...
                    "messages": [AIMessage(content="E2B execution complete")],
                }

            error_context = (
                f"Attempt {attempt + 1} failed - {reason}\n"
                f"Error: {result.get('error', '')[:500]}"
            )
            if sync_errors:
                error_context = f"{error_context}\nFile sync warnings: {' | '.join(sync_errors)}"
        ...
        logger.error(f"E2B executor failed after {max_attempts} attempts for user {user_id}")
        return {
            "e2b_response": {
                "text": f"[E2B execution failed after {max_attempts} attempts: {error_context}]",
                "failed": True,
                "source": "E2B Sandbox",
            },
            "agent_errors": {"e2b_executor": error_context},
            "agents_completed": ["e2b_executor"],
            "messages": [AIMessage(content="E2B execution failed")],
        }

    @staticmethod
    def _build_e2b_task(task: str, available_files: list[dict]) -> str:
        ...
        for file_meta in available_files:
            ...
            filename = file_meta.get("filename") or sandbox_path.rsplit("/", 1)[-1]
            file_lines.append(f"- {filename}: {sandbox_path}")
        ...
        return (
            f"{task}\n\n"
            "Uploaded sandbox files available for this task:\n"
            f"{chr(10).join(file_lines)}\n"
            "Use these exact paths when the task refers to uploaded genotype or data files."
        )
    ...
    def aggregate_responses(self, state: AgentState) -> Dict[str, Any]:
        ...
        # ---------------- E2B Action Agent ----------------
        e2b_resp = state.get("e2b_response")
        if e2b_resp and not e2b_resp.get("failed"):
            text_content = e2b_resp.get("text", "")
            if text_content:
                agent_outputs.append({
                    "agent": "e2b_executor",
                    "source": "E2B Sandbox",
    ...
    def answer_from_graph_summaries(self, query, user_id, resource, token, graph_id=None):
