...
def process_query(current_user_id, auth_token):
    ...
    - Required fields:
        ...
        - json_query: Optional, for advanced queries.
    ...
    try:
        ...
        # Uploaded files: support multiple common field names for compatibility.
        uploaded_files = []
        for key in ("uploaded_files", "context", "file", "files"):
            if key in request.files:
                uploaded_files.extend(
                    [f for f in request.files.getlist(key) if getattr(f, "filename", None)]
                )
        ...
        # If context is posted as a file field, request.form won't contain JSON context.
